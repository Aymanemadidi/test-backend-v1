...
export class TypeUsersService {
  ...
  async create(createTypeUserInput: CreateTypeUserInput): Promise<TypeUser>  {
    const typeuser = await this.TypeUserModel.findOne({ libelle: createTypeUserInput.libelle }).exec();
    if (typeuser) {
  ...
  async update(id: string, updateTypeUserInput: UpdateTypeUserInput): Promise<TypeUser> {
    const typeuser = await this.TypeUserModel.findOne({ libelle: updateTypeUserInput.libelle }).exec();
    if (typeuser) {
      throw new BadRequestException("Ce type d'utilisateur existe déjà");
    }
